...
const initialRuns = rawRunHistory as RunEntry[];

export default function TelemetryDashboard() {
  const [runs, setRuns] = useState<RunEntry[]>(initialRuns);
  const [selectedDate, setSelectedDate] = useState<string>(
    initialRuns.length > 0 ? initialRuns[initialRuns.length - 1].timestamp.split("T")[0] : ""
  );
  const [selectedRunId, setSelectedRunId] = useState<string>(
    initialRuns.length > 0 ? initialRuns[initialRuns.length - 1].id : ""
  );
  ...
  useEffect(() => {
    // --- AUTOMATED REAL-TIME YOUTUBE DELETION AUDIT (CLIENT-SIDE) ---
    initialRuns.forEach((run) => {
      if (run.youtube_url && run.status === "SUCCESS") {
        const videoId = run.youtube_url.split("/").pop()?.split("?")[0];
        if (videoId) {
          fetch(`https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=${videoId}&format=json`, { method: "HEAD" })
            .then((res) => {
              if (res.status === 404 || res.status === 400) {
                console.warn(`[AUTO-DELETION AUDIT] Video ${videoId} (Run #${run.github_run_number}) deleted on YouTube! Updating status to FAILED.`);
                setRuns((prev) =>
                  prev.map((r) =>
                    r.id === run.id
                      ? {
                          ...
                          youtube_url: null,
                          youtube_stats: null,
                          error_traceback: "Video automatically detected as deleted/removed from YouTube channel."
                        }
                      : r
                  )
                );
  ...
  const getRunWinningScore = (run?: RunEntry): number => {
    if (!run) return 8.5;
  ...
  const selectedRunWinningScore = getRunWinningScore(selectedRun).toFixed(2);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        {isUploadWorkflow ? (
          <>
            ...
            <div className="bg-[#131b2e] p-5 rounded-2xl border border-[#1f2d4d] space-y-2">
              <div className="flex justify-between items-center text-gray-400">
                <span className="text-xs font-bold uppercase tracking-wider">Selected Run Auto-QA Score</span>
                <Sparkles className="w-4 h-4 text-[#FFBF00]" />
              </div>
              <div className="text-3xl font-extrabold text-[#FFBF00]">{selectedRunWinningScore} / 10</div>
              <span className="text-xs text-gray-400">Run #{selectedRun?.github_run_number || 'N/A'} winning score</span>
            </div>
          </>
        ) : (
          <>
