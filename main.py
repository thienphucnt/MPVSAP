    ...
    for current_model in candidates:
        for attempt in range(max_retries):
            try:
                ...
                # If we hit quota/rate limits or a model is not found, break out of the retry loop of the current model and try the next model in the fallback chain
                # Added 404 to also trigger fallback, as this can indicate a model is not available for the specific API version/project.
                if is_quota_or_rate_limit or "404 NOT_FOUND" in str(e).upper():
                    print(f"Model {current_model} encountered an issue (quota, rate limit, or not found). Moving to next fallback model...")
                    break # Break inner loop, move to next model in candidates
                ...
                if is_transient and attempt < max_retries - 1:
                    ...
                    raise
        if success: # If a model succeeded, we don't need to try further candidates
            break
...
def generate_content(client: genai.Client, category: str, recent_topics: List[str]) -> Tuple[str, List[str], str, str, str]:
    # Changed to use a stable and widely available Gemini model for primary generation.
